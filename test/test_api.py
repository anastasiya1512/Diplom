...
import requests
import json
from Pages.settings import BASE_URL_API, KINOPOISK_API_KEY
...
class TestAPI:
    ...
    @allure.title("Получение списка фильмов по жанру 'боевик'")
    def test_get_movies_by_genre(self):
        ...
        with allure.step("Проверка наличия фильмов и соответствия жанру"):
            ...
            assert isinstance(data["docs"], list), "'docs' должен быть списком"
            assert len(data["docs"]) > 0, "Список фильмов по жанру не пуст"
